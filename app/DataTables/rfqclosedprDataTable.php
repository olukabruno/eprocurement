...
class rfqclosedprDataTable extends DataTable
{
       /**
    ...
    public function dataTable($query)
    {
        return datatables($query)
            ->addColumn('action', function ($rfq) {
                return '<a href="' . route("rfq.generate", $rfq->id) . '" title="Generate RFQ Form" class="btn btn-primary btn-xs glyphicon glyphicon-plus"></a>';
            })
    ...
    {
        $queries = $model->newQuery()->where('created_rfq', 0)
        ->where('is_supplemental', 0)->where('status',"Closed")
    ...
    {
        return 'rfqclosedpr_' . date('YmdHis');
    }
}
